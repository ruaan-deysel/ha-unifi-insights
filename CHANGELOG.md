...
- `UniFiResponseError.__str__()` now returns a safe representation (`status=<code>`) without exposing the raw API response body
...
- Fixed incorrect `unique_id` values in remote config entry test fixtures — remote entries now use `console_id` and local entries use the host URL, matching the actual config flow behavior
