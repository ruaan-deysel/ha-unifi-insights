...
async def test_raw_nvr_api() -> dict | None:
    """Make raw API call to get NVR data."""
    ...
    endpoints_to_try = [
        "/proxy/protect/api/bootstrap",  # Classic bootstrap endpoint
        "/proxy/protect/integration/v1/nvr",  # Integration API NVR
        "/api/protect/bootstrap",  # Alternative
    ]
...
async def test_protect_api(client: UniFiProtectClient) -> dict:
    ...
    # Try raw API call to get NVR bootstrap data
    print("\n🔧 RAW API call for NVR data:")
    try:
        raw_nvr = await test_raw_nvr_api()
        if raw_nvr:
            results["nvr_raw"] = raw_nvr
            print(f"  ✅ Raw API returned data!")
            print(f"  Keys: {list(raw_nvr.keys())}")
            # Print storage-related fields
            for key in raw_nvr.keys():
                if "storage" in key.lower() or "disk" in key.lower():
                    print(f"  {key}: {raw_nvr.get(key)}")
            # Print other important fields
            print(f"  name: {raw_nvr.get('name')}")
            print(f"  version: {raw_nvr.get('version')}")
            print(f"  host: {raw_nvr.get('host')}")
            print(f"  mac: {raw_nvr.get('mac')}")
    except Exception as e:
    ...
    try:
        ...
        # Print ALL storage-related fields for debugging
        print("\n  📦 Storage Fields (for debugging):")
        storage_keys = [k for k in nvr_dict.keys() if "storage" in k.lower()]
        for key in storage_keys:
            print(f"    {key}: {nvr_dict.get(key)}")

        # Check for nested storageInfo
        if storage := nvr_dict.get("storageInfo"):
            ...
            print(f"  Storage Summary: {used_gb:.1f} GB / {total_gb:.1f} GB")

        # Print all NVR keys for reference
        print("\n  🔑 All NVR Keys:")
        for key in sorted(nvr_dict.keys()):
            value = nvr_dict.get(key)
